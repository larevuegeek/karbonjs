...
import type { BrandProvider, BrandButtonVariant, ButtonSize, ButtonShape } from '@karbonjs/ui-core'
...
function getIcon(brand: string): { svg: string; hex: string; viewBox: string } | null {
...
export function ButtonBrand({
  ...
  const icon = useMemo(() => getIcon(brand), [brand])
  const brandLabel = meta[brand]?.label || brand
  const brandFg = meta[brand]?.fg || 'white'

  const rawHex = icon ? `#${icon.hex}` : '#666'
  const isDark = luminance(rawHex) < 0.25
  const visibleColor = isDark ? lighten(rawHex, 0.6) : rawHex
  const solidBg = isDark ? lighten(rawHex, 0.15) : rawHex
  const solidBgHover = isDark ? lighten(rawHex, 0.25) : rawHex + 'cc'

  const inlineStyle = useMemo((): React.CSSProperties => {
    let bg: string, bgH: string, c: string, cH: string, b: string, bH: string

    switch (variant) {
      case 'solid':
        bg = solidBg; bgH = solidBgHover; c = brandFg; cH = brandFg; b = 'none'; bH = 'none'
        break
      case 'outline':
        bg = 'transparent'; bgH = `${visibleColor}15`; c = visibleColor; cH = visibleColor
        b = `1px solid ${visibleColor}40`; bH = `1px solid ${visibleColor}80`
        break
      case 'light':
        bg = `${visibleColor}15`; bgH = `${visibleColor}25`; c = visibleColor; cH = visibleColor
        b = 'none'; bH = 'none'
        break
      default:
        bg = solidBg; bgH = solidBgHover; c = brandFg; cH = brandFg; b = 'none'; bH = 'none'
    ...
    const isH = !isDisabled && (active || hovered)
    ...
    return style
  }, [variant, solidBg, solidBgHover, brandFg, visibleColor, hovered, active, isDisabled])

  const sizeClass = shape === 'circle' && iconOnly ? circleSizes[size] : sizeClasses[size]
  ...
  return (
    ...
    >
      {loading ? (
        <>
          <svg xmlns="http://www.w3.org/2000/svg" width={icoSize} height={icoSize} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="animate-spin"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
        ...
        <>
          ...
          {!iconOnly && (
            children ? children : <span>{brandLabel}</span>
          )}
        </>
      )}
    </button>
