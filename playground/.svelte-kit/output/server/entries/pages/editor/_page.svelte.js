import { T as escape_html, c as stringify, n as attr_style } from "../../../chunks/server.js";
import { _ as CodeBlock, r as RichTextEditor } from "../../../chunks/src.js";
import { t as DemoSection } from "../../../chunks/DemoSection.js";
//#region src/routes/editor/+page.svelte
...
<RichTextEditor bind:value={content} {media} placeholder="Redigez..." />`;
	...
	const demoTokens = [
		...
		},
		{
			...
			icon: "➖",
			group: "Contenu",
			template: "<div style=\"margin:24px 0;text-align:center;\"><span style=\"display:inline-block;width:60px;height:3px;border-radius:2px;background:var(--karbon-primary);\"></span></div>"
		},
		{
			id: "spacer",
			label: "Espace",
			icon: "↕️",
			group: "Contenu",
			template: "<div data-token=\"spacer\" style=\"height:48px;margin:0;\"></div>"
		},
		{
			id: "cta",
		...
		},
		{
			...
			icon: "✨",
			group: "Marketing",
			template: "<div data-token=\"features\" style=\"display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:12px 0;\"><div style=\"padding:16px;border-radius:10px;background:var(--karbon-bg-2);\"><div style=\"font-size:20px;margin-bottom:6px;\">🚀</div><div style=\"font-size:13px;font-weight:600;color:var(--karbon-text);margin-bottom:4px;\">Rapide</div><div style=\"font-size:12px;color:var(--karbon-text-3);\">Performance optimale.</div></div><div style=\"padding:16px;border-radius:10px;background:var(--karbon-bg-2);\"><div style=\"font-size:20px;margin-bottom:6px;\">🎨</div><div style=\"font-size:13px;font-weight:600;color:var(--karbon-text);margin-bottom:4px;\">Design</div><div style=\"font-size:12px;color:var(--karbon-text-3);\">Interface soignee.</div></div><div style=\"padding:16px;border-radius:10px;background:var(--karbon-bg-2);\"><div style=\"font-size:20px;margin-bottom:6px;\">🔒</div><div style=\"font-size:13px;font-weight:600;color:var(--karbon-text);margin-bottom:4px;\">Securise</div><div style=\"font-size:12px;color:var(--karbon-text-3);\">Protection totale.</div></div><div style=\"padding:16px;border-radius:10px;background:var(--karbon-bg-2);\"><div style=\"font-size:20px;margin-bottom:6px;\">💬</div><div style=\"font-size:13px;font-weight:600;color:var(--karbon-text);margin-bottom:4px;\">Support</div><div style=\"font-size:12px;color:var(--karbon-text-3);\">24/7 disponible.</div></div></div>"
		}
	];
	const media = {
		browse: async () => ({
	...
	function $$render_inner($$renderer) {
		$$renderer.push(`<h1 class="text-3xl font-bold mb-2">RichTextEditor</h1> <p class="text-[var(--karbon-text-3)] mb-10">Editeur WYSIWYG complet avec toolbar, formatage, medias, source view et menu contextuel.</p> <div class="rounded-xl p-6 mb-8" style="background:var(--karbon-bg-2);border:1px solid var(--karbon-border);"><h2 class="text-lg font-semibold mb-3">Utilisation</h2> `);
		CodeBlock($$renderer, {
			code: usageCode,
			language: "svelte",
		...
		$$renderer.push(`<!----></div> `);
		{
			function children($$renderer) {
				...
				$$renderer.push(`<!----> <div class="flex gap-2 mt-4"><button class="text-xs px-3 py-1.5 rounded-lg cursor-pointer transition-colors"${attr_style(`background:${stringify("var(--karbon-bg-card)")};color:${stringify("var(--karbon-text-2)")};border:1px solid var(--karbon-border);`)}>${escape_html("Voir")} le HTML</button> <button class="text-xs px-3 py-1.5 rounded-lg cursor-pointer transition-colors"${attr_style(`background:${stringify("var(--karbon-bg-card)")};color:${stringify("var(--karbon-text-2)")};border:1px solid var(--karbon-border);`)}>${escape_html("Voir")} le rendu</button></div> `);
				$$renderer.push("<!--[-1-->");
				$$renderer.push(`<!--]--> `);
				$$renderer.push("<!--[-1-->");
				$$renderer.push(`<!--]-->`);
			...
			DemoSection($$renderer, {
				title: "Editeur complet",
				description: "Toolbar avec formatage, insertion d'images, tableaux, code blocks et plus.",
				code: `<RichTextEditor bind:value={content} {media} />`,
				children,
				$$slots: { default: true }
			});
		}
		$$renderer.push(`<!----> `);
		{
			...
			DemoSection($$renderer, {
				...
				description: "Selectionnez du texte pour voir la toolbar apparaitre. Mode ideal pour une experience d'ecriture immersive.",
				code: `<RichTextEditor bind:value={floatingContent} toolbarMode="floating" {media} />`,
				children,
				$$slots: { default: true }
			});
		}
		$$renderer.push(`<!----> `);
		{
			...
			DemoSection($$renderer, {
				title: "Tokens / Blocs custom",
				description: "Cliquez sur le bouton + dans la toolbar pour inserer des blocs pre-configures.",
				code: `<RichTextEditor bind:value={tokenContent} tokens={demoTokens} {media} />`,
				children,
				$$slots: { default: true }
			});
		}
		...
		{
			function children($$renderer) {
				$$renderer.push(`<div class="space-y-6"><div><span class="text-[11px] font-semibold uppercase tracking-wider mb-2 block" style="color:var(--karbon-text-4);">Minimal (gras, italique, souligne, lien)</span> `);
				RichTextEditor($$renderer, {
					value: "<p>Editeur minimal — juste l'essentiel.</p>",
					toolbar: "minimal",
					media
				});
				$$renderer.push(`<!----></div> <div><span class="text-[11px] font-semibold uppercase tracking-wider mb-2 block" style="color:var(--karbon-text-4);">Standard (defaut)</span> `);
				RichTextEditor($$renderer, {
					value: "<p>Editeur standard — le set de base.</p>",
					toolbar: "standard",
					media
				});
				$$renderer.push(`<!----></div> <div><span class="text-[11px] font-semibold uppercase tracking-wider mb-2 block" style="color:var(--karbon-text-4);">Full (tout)</span> `);
				RichTextEditor($$renderer, {
					value: "<p>Editeur complet — toutes les options.</p>",
					toolbar: "full",
					media
				});
			...
			DemoSection($$renderer, {
				...
				description: "3 presets pour adapter la toolbar au contexte.",
				code: `<RichTextEditor value="..." toolbar="minimal" {media} />
