<script lang="ts">
  ...
  const themes = ['default', 'midnight', 'aurora', 'minimal', 'corporate', 'sunset', 'ocean', 'forest', 'neon', 'rose'];

  // SVG icons (Lucide 14x14)
  const i = (d: string) => `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${d}</svg>`;
  const icons = {
  ...
  const nav = [
    ...
    { group: 'Foundation', items: [
      { label: 'Couleurs', href: '/colors', icon: icons.palette },
      { label: 'Themes', href: '/themes', icon: icons.sun },
    ]},
    { group: 'Actions', items: [
      { label: 'Button', href: '/button', icon: icons.click },
      { label: 'ButtonBrand', href: '/buttonbrand', icon: icons.link },
    ]},
    { group: 'Formulaires', items: [
      { label: 'Input', href: '/input', icon: icons.type },
      { label: 'Select', href: '/select', icon: icons.list },
      { label: 'Checkbox & Toggle', href: '/checkbox', icon: icons.check },
      { label: 'Textarea', href: '/textarea', icon: icons.text },
      { label: 'Slider', href: '/slider', icon: icons.sliders },
      { label: 'Radio', href: '/radio', icon: icons.circle },
    ]},
    { group: 'Data', items: [
      { label: 'DataTable', href: '/datatable', icon: icons.table },
      { label: 'Pagination', href: '/pagination', icon: icons.hash },
    ]},
    { group: 'Feedback', items: [
      { label: 'Badge', href: '/badge', icon: icons.tag },
      { label: 'Alert', href: '/alert', icon: icons.alert },
      { label: 'Progress', href: '/progress', icon: icons.bar },
      { label: 'Avatar', href: '/avatar', icon: icons.user },
      { label: 'Skeleton', href: '/skeleton', icon: icons.bone },
      { label: 'Tooltip', href: '/tooltip', icon: icons.msg },
    ]},
    { group: 'Overlay', items: [
      { label: 'Modal', href: '/modal', icon: icons.layers },
      { label: 'Dialog', href: '/dialog', icon: icons.helpCircle },
      { label: 'Toast', href: '/toast', icon: icons.bell },
      { label: 'ImgBox', href: '/imgbox', icon: icons.image },
    ]},
    { group: 'Navigation', items: [
      { label: 'Tabs', href: '/tabs', icon: icons.tabs },
      { label: 'Accordion', href: '/accordion', icon: icons.chevDown },
      { label: 'Breadcrumb', href: '/breadcrumb', icon: icons.link },
      { label: 'Dropdown', href: '/dropdown', icon: icons.chevRight },
    ]},
    { group: 'Layout', items: [
      { label: 'Card', href: '/card', icon: icons.card },
      { label: 'Divider', href: '/divider', icon: icons.minus },
      { label: 'PageHeader', href: '/pageheader', icon: icons.pin },
      { label: 'EmptyState', href: '/emptystate', icon: icons.inbox },
    ]},
    { group: 'Media', items: [
      { label: 'Image', href: '/image', icon: icons.image },
      { label: 'ImageCompare', href: '/imagecompare', icon: icons.image },
      { label: 'Carousel', href: '/carousel', icon: icons.carousel },
    ]},
    { group: 'Editeur', items: [
    ...
    { group: 'Divers', items: [
      { label: 'Kbd', href: '/kbd', icon: icons.kbd },
    ]},
  ];
...
<div class="flex h-screen overflow-hidden bg-[var(--karbon-bg)] text-[var(--karbon-text)]">
  <!-- Sidebar -->
  <aside class="shrink-0 h-full overflow-y-auto border-r border-[var(--karbon-border)] bg-[var(--karbon-bg-2)] transition-all duration-200"
    style="width: {collapsed ? '0' : '260px'}"
  >
    <div class="p-4 border-b border-[var(--karbon-border)]">
      <div class="flex items-center gap-2">
        <span class="text-xl font-bold">⚡</span>
        <div>
          <div class="font-bold text-sm">KarbonJS</div>
          <div class="text-[10px] text-[var(--karbon-text-4)]">v0.2.5 — Playground</div>
        </div>
      </div>
    </div>

    <!-- Theme selector + Dark/Light toggle -->
    <div class="p-3 border-b border-[var(--karbon-border)]">
      <div class="flex items-center gap-2">
        <select
          class="flex-1 text-xs rounded-lg px-2.5 py-1.5 pr-7 outline-none cursor-pointer"
        ...
        <button
          ...
          class="shrink-0 p-2 rounded-lg cursor-pointer transition-all"
          style="background: var(--karbon-bg-card); border: 1px solid var(--karbon-border); color: var(--karbon-text-2);"
    ...
    <nav class="p-2 text-sm">
      {#each nav as item}
        {#if 'href' in item}
          <a
            href={item.href}
            class="flex items-center gap-2.5 px-3 py-2 rounded-lg transition-colors mb-0.5
              {isActive(item.href) ? 'bg-[var(--karbon-primary)]/15 text-[var(--karbon-primary)]' : 'text-[var(--karbon-text-2)] hover:bg-[var(--karbon-nav-hover-bg)]'}"
          >
            <span class="shrink-0 opacity-60">{@html item.icon}</span>
            <span class="font-medium">{item.label}</span>
          </a>
        {:else}
          <div class="mt-4 mb-1 px-3">
            <span class="text-[10px] font-semibold uppercase tracking-widest text-[var(--karbon-text-4)]">{item.group}</span>
          </div>
          {#each item.items as sub}
            <a
              href={sub.href}
              class="flex items-center gap-2.5 px-3 py-1.5 rounded-lg transition-colors mb-0.5
                {isActive(sub.href) ? 'bg-[var(--karbon-primary)]/15 text-[var(--karbon-primary)] font-semibold' : 'text-[var(--karbon-text-2)] hover:bg-[var(--karbon-nav-hover-bg)]'}"
            >
              <span class="shrink-0 opacity-50">{@html sub.icon}</span>
              <span>{sub.label}</span>
            </a>
          {/each}
        {/if}
  ...
  <main class="flex-1 overflow-y-auto">
    <header class="sticky top-0 z-10 flex items-center gap-3 px-6 py-3 border-b border-[var(--karbon-border)] bg-[var(--karbon-bg)]/80 backdrop-blur-lg">
      <button
        onclick={() => collapsed = !collapsed}
        class="p-1.5 rounded-md text-[var(--karbon-text-3)] hover:bg-[var(--karbon-nav-hover-bg)] cursor-pointer transition-colors"
      ...
      <span class="text-sm font-medium text-[var(--karbon-text-2)]">
        Theme: <span class="text-[var(--karbon-primary)] font-semibold">{currentTheme}</span>
        · {darkMode ? 'Dark' : 'Light'}
      </span>
    </header>
